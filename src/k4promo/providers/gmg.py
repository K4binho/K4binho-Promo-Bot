...
IMPORTANTE: impact.com NAO e a antiga CJ Affiliate; sao redes diferentes.
Internamente esta integracao e identificada como "Impact". As variaveis
CJ_ACCOUNT_SID / CJ_AUTH_TOKEN continuam aceitas apenas como aliases de
compatibilidade para IMPACT_ACCOUNT_SID / IMPACT_AUTH_TOKEN.
...
Credenciais: Account SID / Auth Token gerados no painel da impact.com
(User profile -> Settings -> Technical -> API).
...
def fetch_catalog_items(
    ...
    *,
    ...
    Percorre ate `max_pages` paginas de `page_size` itens e, se `currency`
    for informada, mantem apenas itens naquela moeda (campo `Currency`)."""
    items: list[dict] = []
    for page in range(1, max(1, max_pages) + 1):
        ...
        items.extend(batch)
        if len(batch) < page_size or not data.get("@nextpageuri"):
            break
    if currency:
        wanted = currency.upper()
        filtered = [i for i in items if str(i.get("Currency") or wanted).upper() == wanted]
        return filtered
    return items
...
if __name__ == "__main__":
    ...
    sid = os.environ.get("IMPACT_ACCOUNT_SID") or os.environ.get("CJ_ACCOUNT_SID", "")
    token = os.environ.get("IMPACT_AUTH_TOKEN") or os.environ.get("CJ_AUTH_TOKEN", "")

    if not (sid and token):
        raise SystemExit("Defina IMPACT_ACCOUNT_SID e IMPACT_AUTH_TOKEN no .env (credenciais da impact.com).")
