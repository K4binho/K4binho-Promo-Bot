...
def score(
    ...
    from k4promo.storage.price_history import history_confidence
...
def score_store_deal(
    ...
    """Score comercial genérico para lojas sem histórico próprio (Shopee,
    KaBuM). Mesma estrutura do score AliExpress, mas a confiança usa
    vendas e avaliação quando a fonte oferece esses dados."""
    from k4promo.storage.price_history import history_confidence
    ...
    if sales_count >= 1000:
        conf_pts += 40
        conf_reasons.append(f"volume alto {source} ({sales_count} vendas) (+40)")
    elif sales_count >= 100:
        conf_pts += 20
        conf_reasons.append(f"volume razoavel {source} ({sales_count} vendas) (+20)")
    elif sales_count >= 10:
        conf_pts += 5
        conf_reasons.append(f"poucas vendas {source} ({sales_count}) (+5)")
    if rating is not None and rating >= 4.5:
        conf_pts += 20
        conf_reasons.append(f"avaliacao {rating:.1f} (+20)")
    elif rating is not None and rating >= 4.0:
        conf_pts += 10
        conf_reasons.append(f"avaliacao {rating:.1f} (+10)")
    if sales_count < 10 and rating is None:
        conf_pts -= 10
        conf_reasons.append(f"sem prova social {source} (-10)")
    conf_pts = _clamp(conf_pts)
